...
|new| New model-wide constraint that can be applied to all or a subset of locations and technologies in a model, initially covering "energy_cap_min", "energy_cap_max", "demand_share_min", and "demand_share_max". Model-wide constraints supersede the `group_share` constraints, which are now deprecated and will be removed in v0.7.0.
