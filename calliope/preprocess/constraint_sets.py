...
def generate_constraint_sets(model_run):
    ...
    for sense in ['min', 'max', 'equals']:
        constraint_sets[f'techlists_group_share_energy_cap_{sense}_constraint'] = [
            ...
            if f'energy_cap_{sense}' in model_run.model.get_key('group_share.{}'.format(i), {}).keys()
        ]
        constraint_sets[f'techlists_carrier_group_share_carrier_prod_{sense}_constraint'] = [
            i + '::' + carrier
            for i in sets.techlists
            if f'carrier_prod_{sense}' in model_run.model.get_key('group_share.{}'.format(i), {}).keys()
            for carrier in sets.carriers
            if carrier in model_run.model.get_key(f'group_share.{i}.carrier_prod_{sense}', {}).keys()
        ]
