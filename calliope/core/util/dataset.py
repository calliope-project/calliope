...
def get_loc_techs(loc_techs, tech=None, loc=None):
    ...
    if tech and not loc:
        ...
        relevant_loc_techs = [
            "::".join(i)
            for i in possible_loc_techs.intersection(loc_techs_set)
        ]
    else:
...
def reorganise_xarray_dimensions(data):
    ...
    And (TODO) delete any empty dimensions (and variables that are empty as a result)
    """
    ...
    updated_data = data.transpose(*new_dims).reindex(
        {k: data[k] for k in new_dims}
    )

    # TODO: make the below work in this function (it came from preprocessing.checks)
...
def split_loc_techs(data_var, return_as="DataArray"):
    ...
    if return_as == "MultiIndex DataArray":
        ...
        return reorganise_xarray_dimensions(
            updated_data_var.unstack()
        ).to_series()
