...
def add_time_dimension(data, model_run):
    ...
    for variable in data.data_vars:
        ...
        # 3) get Series of all uses of 'file=' or 'df=' for this variable (dataframe keys)
        dfkeys = data_series[data_series.str.contains('file=') | data_series.str.contains('df=')]
        ...
        dfkeys = dfkeys.str.split('=').str[1].str.rsplit(':', 1)
        if isinstance(dfkeys.index, pd.MultiIndex):
            dfkeys.index = dfkeys.index.remove_unused_levels()

        # 6) Get all timeseries data from dataframes stored in model_run
        timeseries_data = []
        key_errors = []
        for loc_tech, (dfkey, column) in dfkeys.iteritems():
            try:
                timeseries_data.append(model_run.timeseries_data[dfkey].loc[:, column].values)
            except KeyError:
                key_errors.append(
                    'column `{}` not found in dataframe `{}`, but was requested by '
                    'loc::tech `{}`.'.format(column, dfkey, loc_tech)
                )
        if key_errors:
            exceptions.print_warnings_and_raise_errors(errors=key_errors)

        timeseries_data_series = pd.DataFrame(index=dfkeys.index,
