...
def load_constraints(backend_model):
    ...
    if 'loc_techs_energy_capacity_min_purchase_milp_constraint' in sets:
        backend_model.energy_capacity_min_purchase_milp_constraint_upper = po.Constraint(
            backend_model.loc_techs_energy_capacity_min_purchase_milp_constraint,
            rule=energy_capacity_min_purchase_milp_constraint_rule_upper
        )
        backend_model.energy_capacity_min_purchase_milp_constraint_lower = po.Constraint(
            backend_model.loc_techs_energy_capacity_min_purchase_milp_constraint,
            rule=energy_capacity_min_purchase_milp_constraint_rule_lower
        )
        backend_model.energy_capacity_min_milp_constraint = po.Constraint(
            backend_model.loc_techs_energy_capacity_min_purchase_milp_constraint,
            rule=energy_capacity_min_milp_constraint_rule
        )
...
def energy_capacity_min_purchase_milp_constraint_rule_lower(backend_model, loc_tech):
    """
    ...
    energy_cap_scale = get_param(backend_model, 'energy_cap_scale', loc_tech)
    return (backend_model.energy_cap[loc_tech] - energy_cap_min) <= (
        energy_cap_scale * backend_model.purchased[loc_tech] * backend_model.bigM
    )

def energy_capacity_min_purchase_milp_constraint_rule_upper(backend_model, loc_tech):
    """
    Set minimum energy capacity decision variable upper bound as a function of
    binary purchase variable
    ...
    energy_cap_scale = get_param(backend_model, 'energy_cap_scale', loc_tech)
    return backend_model.purchased[loc_tech] <= (
        (backend_model.energy_cap[loc_tech] - energy_cap_min) * energy_cap_scale 
    )   
    
def energy_capacity_min_milp_constraint_rule(backend_model, loc_tech):
    """
    Set minimum energy capacity decision variable upper bound as a function of
    binary purchase variable
    ...
    energy_cap_scale = get_param(backend_model, 'energy_cap_scale', loc_tech)
    return backend_model.energy_cap[loc_tech] >= (
        energy_cap_min * energy_cap_scale
    )
