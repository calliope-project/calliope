...
from calliope.backend.pyomo.util import loc_tech_is_in, get_param, is_valid
...
def demand_share_per_timestep_decision_main_constraint_rule(
    ...
    if not is_valid(share_of_carrier_demand):
...
def demand_share_per_timestep_decision_sum_constraint_rule(
 ...
 .. container:: scrolling-wrapper
    ...
    if not is_valid(share_of_carrier_demand) or np.isinf(share_of_carrier_demand):
