...
class TestParsingForEach:
    @pytest.fixture(
        params=[
            ...
            (
                "[a in A, a1 in A1, a_2 in A_2, foo in foos]",
                ["a", "a1"],
                ["A", "A1"],
                ["A_2", "foos"],
    ...
    @pytest.fixture
    def foreach_parser(self, dummy_parsed_constraint, dummy_model_data):
        return dummy_parsed_constraint._foreach_parser(dummy_model_data.dims)
    ...
    def test_parse_foreach_to_sets_unknown_set(self, constraint_data):
        ...
        if len(missing_sets) == 0:
            ...
            assert check_error_or_warning(
                constraint_obj._errors, "Expected A1 | A_1 | A | techs"
            )
