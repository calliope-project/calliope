...
class TestOptimisationConfigOverrides:
    ...
    def storage_inter_cluster(self,):
        return build_model({"model.optimisation_config_overrides": ["storage_inter_cluster"]}, "simple_supply,two_hours,investment_costs")
    ...
    def test_internal_override(self,storage_inter_cluster):
        assert "set_storage_initial_inter_cluster" in storage_inter_cluster.component_config["constraints"].keys()

    def test_variable_bound(self,storage_inter_cluster):
        assert storage_inter_cluster.component_config["variables"]["storage"]["bounds"]["min"] == np.inf

    @pytest.mark.parametrize(["override","expected"], [(["foo"],["foo"]),(["bar","foo"], ["bar","foo"]), (["foo","storage_inter_cluster"], ["foo"])])
    def test_allowed_internal_constraint(self,override, expected):
        with pytest.raises(calliope.exceptions.ModelError) as excinfo:
            build_model({"model.optimisation_config_overrides": override}, "simple_supply,two_hours,investment_costs")
        assert check_error_or_warning(
            excinfo,
    ...
    def test_internal_override_from_yaml(self,temp_path):
        new_constraint = calliope.AttrDict({"constraints":{"constraint_name":{"foreach": [], "where": "", "equation": ""}}})
        new_constraint.to_yaml(temp_path.join("custom-constraints.yaml"))
        m = build_model({"model.optimisation_config_overrides": [temp_path.join("custom-constraints.yaml")]}, "simple_supply,two_hours,investment_costs")
    ...
    def test_override_existing_internal_constraint(self,temp_path, simple_supply):
        ...
        new_constraint = calliope.AttrDict({"constraints":{"energy_capacity_per_storage_capacity_min":{"foreach": ["nodes"]}}})
        new_constraint.to_yaml(file_path)
        m = build_model({"model.optimisation_config_overrides": [file_path]}, "simple_supply,two_hours,investment_costs")
        base = simple_supply.component_config["constraints"]["energy_capacity_per_storage_capacity_min"]
        new = m.component_config["constraints"]["energy_capacity_per_storage_capacity_min"]
    ...
    def test_override_order(self,temp_path, simple_supply):
        ...
        new_constraint_1 = calliope.AttrDict({"constraints":{"energy_capacity_per_storage_capacity_min":{"foreach": ["nodes"]}}})
        new_constraint_1.to_yaml(file_path_1)
        new_constraint_2 = calliope.AttrDict({"constraints":{"energy_capacity_per_storage_capacity_min":{"foreach": ["techs"]}}})
        new_constraint_2.to_yaml(file_path_2)
        m = build_model({"model.optimisation_config_overrides": [file_path_1,file_path_2]}, "simple_supply,two_hours,investment_costs")
    ...
    def test_override_existing_internal_constraint_merge(self,temp_path, simple_supply):
        new_constraint = calliope.AttrDict({"variables":{"storage":{"bounds": {"min": -1}}}})
        file_path = temp_path.join("custom-constraints.yaml")
        new_constraint.to_yaml(file_path)
        m = build_model({"model.optimisation_config_overrides": ["storage_inter_cluster", file_path]}, "simple_supply,two_hours,investment_costs")
