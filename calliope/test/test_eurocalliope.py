...
class TestEuroCalliopeConstraints:
    def test_carrier_production_max_time_varying_constraint(self):
        ...
        carrier_prod = get_series(m, "carrier_prod", {"techs": "boiler"}).droplevel('carriers')
        assert (
            carrier_prod <= energy_cap_max.droplevel('techs').mul(energy_cap, level='locs').mul(TOL)
        ).all()
    ...
    def test_chp_extraction_p2h_tech(self):
        ...
        carrier_prod = get_series(
            m, "carrier_prod", {"techs": "chp", "locs": "X1"}
        )
        energy_cap = m._model_data.energy_cap.loc["X1::chp"].item()
        slope = 2.5 / (3 - 1)
        assert carrier_prod.sum() > 0
        assert (
            carrier_prod.xs("electricity") <= slope * (energy_cap * 2.5 - carrier_prod.xs("heat")) * TOL
        ).all()

        assert (
            carrier_prod.xs("electricity") <= (energy_cap - carrier_prod.xs("heat") * 0.4) * TOL
        ).all()
    ...
    def test_chp_extraction_simple_tech(self):
        ...
        assert (TOL * carrier_prod.xs("electricity") >= carrier_prod.xs("heat") * 0.45).all()

        assert (
            carrier_prod.xs("electricity") <= (m._model_data.energy_cap.loc["X1::chp"].item() - carrier_prod.xs("heat") * 0.14) * TOL
        ).all()
    ...
    @pytest.mark.parametrize(
        "scenario",
        ("carrier_prod_per_week", "carrier_prod_per_week_ts")
    )
    def test_carrier_prod_per_week_constraint(self, scenario):
        ...
        for week in m._model_data.weeks.values:
            if "_ts" in scenario:
                _min = 0.0008064516129032258 * (m._model_data.week_numbers == week).sum()
                _max = 0.0014784946236559141 * (m._model_data.week_numbers == week).sum()
            else:
                _min = 0.1
                _max = 0.5
            assert (
                carrier_prod
                .loc[m._model_data.week_numbers == week]
                .sum()
                / carrier_prod.sum()
                >= _min
            )
