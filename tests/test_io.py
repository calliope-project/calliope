...
class TestIO:
    ...
    def vars_to_add_attrs(self):
        return ["source_max", "flow_out_cap"]
    ...
    def test_csv_contents(self, model_csv_dir):
        with open(
            os.path.join(model_csv_dir, "inputs_flow_out_cap_max_systemwide.csv"), "r"
        ) as f:
    ...
    def test_save_csv_no_dropna(self, model):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with open(
                os.path.join(out_path, "inputs_flow_out_cap_max_systemwide.csv"), "r"
            ) as f:
    ...
    def test_save_lp(self, model):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with open(out_path, "r") as f:
                assert "variables(flow_out_cap)" in f.read()
