...
Not all combinations of the set items will contain data; if you did not define your `PV` technology at node `X1` then there will be no data for :python:`model.inputs.flow_out_cap.sel(techs="PV", nodes="X1")`.
In fact, there is likely to be more empty (`NaN`) data points than filled ones.
In Python you can quickly "densify" your data to look at only filled data points: :python:`model.inputs.flow_out_cap.to_series().dropna()`
