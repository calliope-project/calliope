...
The efficiency of the DC to AC inverter (which occurs after conversion from source to carrier) is considered in ``parasitic_eff`` and the ``area_use_per_flow_out_cap`` gives a link between the installed area of solar panels to the installed capacity of those panels (i.e. kWp).
...
The first is ``boiler`` (natural gas boiler), which serves as an example of a simple conversion technology with one input carrier and one output carrier. Its only constraints are the cost of built output capacity (``costs.monetary.flow_out_cap``), a constraint on its maximum built output capacity (``constraints.flow_out_cap_max``), and a carrier conversion efficiency (``constraints.flow_out_eff``).

.. figure:: images/conversion.*
   :alt: Simple conversion node

   The layout of a simple node, in this case ``boiler``, which has one carrier input, one carrier output, a carrier conversion efficiency (:math:`flow_{out}^{eff}`), and a constraint on its maximum built :math:`flow_{out}^{cap}` (which puts an upper limit on :math:`flow_{out}`).
