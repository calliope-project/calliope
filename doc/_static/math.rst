...
.. container:: scrolling-wrapper
    ...
    .. math::
        ...
        \begin{cases}
            ...
            \text{if } (\textit{energy_eff}_\text{node,tech,timestep}\mathord{=}\text{0} \lor \textit{parasitic_eff}_\text{node,tech,timestep}\mathord{=}\text{0})\land{}(\textit{lookup_cluster_first_timestep}\mathord{=}\text{true} \land \neg (\textit{timesteps}_\text{timestep}\mathord{=}\text{timesteps[0]} \land \neg (\text{run_config.cyclic_storage}\mathord{=}\text{true})))
            \\
...
.. container:: scrolling-wrapper
    ...
    .. math::
        ...
        \begin{cases}
            ...
            \text{if } (\textit{energy_eff}_\text{node,tech,timestep}\mathord{>}\text{0})\land{}(\textit{lookup_cluster_first_timestep}\mathord{=}\text{true} \land \neg (\textit{timesteps}_\text{timestep}\mathord{=}\text{timesteps[0]} \land \neg (\text{run_config.cyclic_storage}\mathord{=}\text{true})))
            \\
...
.. container:: scrolling-wrapper
    ...
    .. math::
        \begin{array}{r}
            ...
            \text{ timestep }\negthickspace \in \negthickspace\text{ timesteps }
            \\
            \text{if } (\exists (\textit{export_max}_\text{node,tech}) \land \exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \textit{export}_\text{node,tech}\mathord{=}\text{true})
        \end{array}
        \begin{cases}
...
.. container:: scrolling-wrapper
    ...
    .. math::
        \begin{array}{r}
            ...
            \text{ tech }\negthickspace \in \negthickspace\text{ techs }
            \\
            \text{if } (\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land (\exists (\textit{energy_cap_max}_\text{node,tech}) \lor \exists (\textit{energy_cap_equals}_\text{node,tech})) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{binary})
        \end{array}
        \begin{cases}
...
.. container:: scrolling-wrapper
    ...
    .. math::
        \begin{array}{r}
            ...
            \text{ tech }\negthickspace \in \negthickspace\text{ techs }
            \\
            \text{if } (\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land \exists (\textit{energy_cap_min}_\text{node,tech}) \land \neg (\exists (\textit{energy_cap_equals}_\text{node,tech})) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{binary})
        \end{array}
        \begin{cases}
...
.. container:: scrolling-wrapper
    ...
    .. math::
        \begin{array}{r}
            ...
            \text{ tech }\negthickspace \in \negthickspace\text{ techs }
            \\
            \text{if } (\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land (\exists (\textit{storage_cap_max}_\text{node,tech}) \lor \exists (\textit{storage_cap_equals}_\text{node,tech})) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{binary})
        \end{array}
        \begin{cases}
...
.. container:: scrolling-wrapper
    ...
    .. math::
        \begin{array}{r}
            ...
            \text{ tech }\negthickspace \in \negthickspace\text{ techs }
            \\
            \text{if } (\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land \exists (\textit{storage_cap_min}_\text{node,tech}) \land \neg (\exists (\textit{storage_cap_equals}_\text{node,tech})) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{binary})
        \end{array}
        \begin{cases}
...
.. container:: scrolling-wrapper
    ...
    .. math::
        ...
        \begin{cases}
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})\land{}(\exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \exists (\textit{cost_export}_\text{node,tech,cost,timestep}))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \neg (\text{tech_group=conversion_plus} \lor \text{tech_group=supply_plus} \lor \text{tech_group=supply}) \land \text{carrier_tier} \in \text{[in]})
            \\
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})\land{}(\exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \exists (\textit{cost_export}_\text{node,tech,cost,timestep}))\land{}(\neg (\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep})))
            \\
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})\land{}(\neg (\exists (\textit{cost_export}_\text{node,tech,cost,timestep})))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=supply_plus})
            \\
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})\land{}(\neg (\exists (\textit{cost_export}_\text{node,tech,cost,timestep})))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=supply} \land \textit{energy_eff}_\text{node,tech,timestep}\mathord{>}\text{0} \land \text{carrier_tier} \in \text{[out]})
            \\
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})\land{}(\neg (\exists (\textit{cost_export}_\text{node,tech,cost,timestep})))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})
            \\
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \neg (\text{tech_group=conversion_plus}))\land{}(\exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \exists (\textit{cost_export}_\text{node,tech,cost,timestep}))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=supply_plus})
            \\
            ...
            \text{if } (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}) \land \neg (\text{tech_group=conversion_plus}))\land{}(\exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \exists (\textit{cost_export}_\text{node,tech,cost,timestep}))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})
            \\
            ...
            \text{if } (\neg (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep})))\land{}(\exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \exists (\textit{cost_export}_\text{node,tech,cost,timestep}))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=supply_plus})
            \\
            ...
            \text{if } (\neg (\exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep})))\land{}(\exists (\textit{export_carrier}_\text{node,tech,carrier}) \land \exists (\textit{cost_export}_\text{node,tech,cost,timestep}))\land{}(\exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \land \text{tech_group=conversion_plus})
            \\
...
.. container:: scrolling-wrapper
    ...
    .. math::
        ...
        \begin{cases}
            ...
            \text{if } (\exists (\textit{cost_resource_area}_\text{node,tech,cost}) \land (\exists (\textit{resource_area_min}_\text{node,tech}) \lor \exists (\textit{resource_area_max}_\text{node,tech}) \lor \exists (\textit{resource_area_equals}_\text{node,tech}) \lor \exists (\textit{resource_area_per_energy_cap}_\text{node,tech}) \lor \textit{resource_unit}_\text{node,tech}\mathord{=}\text{energy_per_area}))\land{}(\text{tech_group=transmission})\land{}(\exists (\textit{cost_energy_cap}_\text{node,tech,cost}))\land{}(\exists (\textit{cost_resource_cap}_\text{node,tech,cost}) \land \text{tech_group=supply_plus})\land{}(\exists (\textit{cost_storage_cap}_\text{node,tech,cost}) \land (\text{tech_group=supply_plus} \lor \text{tech_group=storage}))\land{}(\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{integer})
            \\
            ...
            \text{if } (\exists (\textit{cost_resource_area}_\text{node,tech,cost}) \land (\exists (\textit{resource_area_min}_\text{node,tech}) \lor \exists (\textit{resource_area_max}_\text{node,tech}) \lor \exists (\textit{resource_area_equals}_\text{node,tech}) \lor \exists (\textit{resource_area_per_energy_cap}_\text{node,tech}) \lor \textit{resource_unit}_\text{node,tech}\mathord{=}\text{energy_per_area}))\land{}(\text{tech_group=transmission})\land{}(\exists (\textit{cost_energy_cap}_\text{node,tech,cost}))\land{}(\exists (\textit{cost_resource_cap}_\text{node,tech,cost}) \land \text{tech_group=supply_plus})\land{}(\exists (\textit{cost_storage_cap}_\text{node,tech,cost}) \land (\text{tech_group=supply_plus} \lor \text{tech_group=storage}))\land{}(\neg (\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land (\textit{cap_method}_\text{node,tech}\mathord{=}\text{binary} \lor \textit{cap_method}_\text{node,tech}\mathord{=}\text{integer})))
            \\
            ...
            \text{if } (\exists (\textit{cost_resource_area}_\text{node,tech,cost}) \land (\exists (\textit{resource_area_min}_\text{node,tech}) \lor \exists (\textit{resource_area_max}_\text{node,tech}) \lor \exists (\textit{resource_area_equals}_\text{node,tech}) \lor \exists (\textit{resource_area_per_energy_cap}_\text{node,tech}) \lor \textit{resource_unit}_\text{node,tech}\mathord{=}\text{energy_per_area}))\land{}(\text{tech_group=transmission})\land{}(\exists (\textit{cost_energy_cap}_\text{node,tech,cost}))\land{}(\exists (\textit{cost_resource_cap}_\text{node,tech,cost}) \land \text{tech_group=supply_plus})\land{}(\neg (\exists (\textit{cost_storage_cap}_\text{node,tech,cost}) \land (\text{tech_group=supply_plus} \lor \text{tech_group=storage})))\land{}(\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{binary})
            \\
            ...
            \text{if } (\exists (\textit{cost_resource_area}_\text{node,tech,cost}) \land (\exists (\textit{resource_area_min}_\text{node,tech}) \lor \exists (\textit{resource_area_max}_\text{node,tech}) \lor \exists (\textit{resource_area_equals}_\text{node,tech}) \lor \exists (\textit{resource_area_per_energy_cap}_\text{node,tech}) \lor \textit{resource_unit}_\text{node,tech}\mathord{=}\text{energy_per_area}))\land{}(\text{tech_group=transmission})\land{}(\exists (\textit{cost_energy_cap}_\text{node,tech,cost}))\land{}(\exists (\textit{cost_resource_cap}_\text{node,tech,cost}) \land \text{tech_group=supply_plus})\land{}(\neg (\exists (\textit{cost_storage_cap}_\text{node,tech,cost}) \land (\text{tech_group=supply_plus} \lor \text{tech_group=storage})))\land{}(\exists (\textit{cost_purchase}_\text{node,tech,cost}) \land \textit{cap_method}_\text{node,tech}\mathord{=}\text{integer})
            \\
...
.. container:: scrolling-wrapper
    ...
    .. math::
        \begin{array}{r}
            ...
            \text{ cost }\negthickspace \in \negthickspace\text{ costs }
            \\
            \text{if } (\exists (\textit{cost_energy_cap}_\text{node,tech,cost}) \lor \exists (\textit{cost_om_annual}_\text{node,tech,cost}) \lor \exists (\textit{cost_om_annual_investment_fraction}_\text{node,tech,cost}) \lor \exists (\textit{cost_purchase}_\text{node,tech,cost}) \lor \exists (\textit{cost_resource_area}_\text{node,tech,cost}) \lor \exists (\textit{cost_resource_cap}_\text{node,tech,cost}) \lor \exists (\textit{cost_storage_cap}_\text{node,tech,cost}) \lor \exists (\textit{cost_export}_\text{node,tech,cost,timestep}) \lor \exists (\textit{cost_om_con}_\text{node,tech,cost,timestep}) \lor \exists (\textit{cost_om_prod}_\text{node,tech,cost,timestep}))
        \end{array}
        \begin{cases}
