...
BASEPATH = Path("src") / "calliope"

NONDEMAND_TECHGROUPS = ["supply", "storage", "conversion", "transmission"]

PREPEND_SNIPPET = """
[:fontawesome-solid-download: Download the base math formulation as a YAML file]({filepath}){{:download}}
"""
...
def on_files(files: list, config: dict, **kwargs):
    ...
    write_file("base.yaml", base_model, files, config)
    ...
    write_file("storage_inter_cluster.yaml", custom_model, files, config)
...
def write_file(
    filename: str, model: calliope.Model, files: list[File], config: dict
) -> None:
    ...
    output_full_filepath.write_text(
        PREPEND_SNIPPET.format(filepath=file_to_download) + math_doc
    )
