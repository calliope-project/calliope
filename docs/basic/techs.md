...
In the above example, we have a capacity limit `flow_cap_max`, conversion efficiency `flow_out_eff`, the life time (used in [levelised cost calculations][total_levelised_cost]), and the resource available for consumption `source_use_max`.
