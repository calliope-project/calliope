...
In the above example, the files `model.yaml`, `locations.yaml` and `techs.yaml` together are the model definition. This definition could be in one file, but it is more readable when split into multiple. We use the above layout in the example models.
...
Calliope allows a modeller to define technologies with arbitrary characteristics by defining one of the abstract base techs as `parent`, which establishes the basic characteristics (decision variables and constraints) applied to the technology:
...
!!! info "Sharing configuration through inheritance"
    ...
    Note that `inherit` is different to setting a `parent`.
...
To read in CSV files, specify e.g., `source: file=filename.csv` to pick the desired CSV file from within the configured timeseries data path (`model.time_data_path`).

By default, Calliope looks for a column in the CSV file with the same name as the node. It is also possible to specify a column to use when setting `source` per node, by giving the column name with a colon following the filename: `source: file=filename.csv:column`
...
By default, Calliope expects time series data in a model to be indexed by ISO 8601 compatible time stamps in the format `YYYY-MM-DD hh:mm:ss`, e.g. `2005-01-01 00:00:00`. This can be changed by setting `model.time_format` based on [`strftime` directives](https://strftime.org/), which defaults to `"ISO8601"`.

For example, the first few lines of a CSV file, called `pv_resource.csv` giving a source potential for two nodes might look like this, with the first column in the file always being read as the date-time index:
...
When running models from python scripts or shells, it is also possible to pass timeseries directly as `pandas` dataframes. This is done by specifying `source: df=tskey` where `tskey` is the key in a dictionary containing the relevant dataframes. For example, if the same timeseries as above is to be passed, a dataframe called `pv_resource` may be in the python namespace:
...
Just like when using CSV files (see above), Calliope looks for a column in the dataframe with the same name as the node. It is also possible to specify a column to use when setting `source` per node, by giving the column name with a colon following the filename: `source: df=tskey:column`.
...
### Nodes and links (`nodes`, `links`)

A model can specify any number of nodes. These nodes are linked together by transmission technologies. By consuming a carrier in one node and outputting it in another, linked node, transmission technologies allow resources to be drawn from the system at a different node from where they are brought into it.
...
Nodes can optionally specify `coordinates` (used in visualisation or to compute distance between them) and must specify `techs` allowed at that node. As seen in the example above, each allowed tech must be listed, and can optionally specify additional node-specific parameters (constraints or costs). If given, node-specific parameters supersede any group constraints a technology defines in the `techs` section for that node.

The `links` section specifies possible transmission links between nodes in the form `node1,node2`:
...
Scenarios and overrides can be used to generate scripts that run a single Calliope model many times, either sequentially, or in parallel on a high-performance cluster (see the section [Generating scripts to repeatedly run variations of a model][generating-scripts-to-repeatedly-run-variations-of-a-model]).

!!! note
    Overrides can also import other files. This can be useful if many overrides are defined which share large parts of model configuration, such as different levels of interconnection between model zones. See :ref:`imports_in_override_groups` for details.<!--TODO-->
